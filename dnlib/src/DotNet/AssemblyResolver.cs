	...
	public class AssemblyResolver : IAssemblyResolver {
		...
		protected virtual IEnumerable<string> FindAssemblies(IAssembly assembly, ModuleDef sourceModule, bool matchExactly) {
			if (assembly.IsContentTypeWindowsRuntime) {
				...
				foreach (var path in FindAssembliesDotNetCore(assembly, matchExactly))
					yield return path;
			}
		...
		IEnumerable<string> FindAssembliesDotNetCore(IAssembly assembly, bool matchExactly) {
			string asmName = UTF8String.ToSystemStringOrEmpty(assembly.Name) + ".dll";
			var baseDirs = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

			if (Path.DirectorySeparatorChar == '\\') {
				string[] pfEnvVars = new string[] { "ProgramW6432", "ProgramFiles", "ProgramFiles(x86)" };
				foreach (var envVar in pfEnvVars) {
					string pf = Environment.GetEnvironmentVariable(envVar);
					if (!string.IsNullOrEmpty(pf)) {
						string shared = Path.Combine(Path.Combine(pf, "dotnet"), "shared");
				...
				string[] unixDirs = new string[] { "/usr/share/dotnet/shared", "/usr/local/share/dotnet/shared", "/opt/dotnet/shared" };
			...
			foreach (var baseDir in baseDirs) {
				string[] appDirs = new string[] { "Microsoft.NETCore.App", "Microsoft.WindowsDesktop.App", "Microsoft.AspNetCore.App" };
				foreach (var appDir in appDirs) {
					try {
						string targetDir = Path.Combine(baseDir, appDir);
						if (Directory.Exists(targetDir)) {
							dirs.AddRange(Directory.GetDirectories(targetDir));
						}
					}
					catch { }
				}
			...
			dirs.Sort((a, b) => {
				...
				if (Version.TryParse(nameA.Split('-')[0], out var vA) && Version.TryParse(nameB.Split('-')[0], out var vB))
			...
			foreach (var dir in dirs) {
				...
				if (matchExactly && assembly.Version != null) {
					if (Version.TryParse(name.Split('-')[0], out var vDir)) {
						if (vDir.Major != assembly.Version.Major || vDir.Minor != assembly.Version.Minor)
							continue;
					}
				}
